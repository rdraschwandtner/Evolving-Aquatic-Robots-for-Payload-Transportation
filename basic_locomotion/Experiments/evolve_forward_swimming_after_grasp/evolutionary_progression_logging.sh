...
python $FILE --run_num="$RUN_NUM" --output_path=$OUT_PATH --validator --eval_time=$EVAL_TIME --gens=0 --num_joints=16
...
do
    echo $j
    python $FILE --run_num="$RUN_NUM" --output_path=$OUT_PATH --validator --eval_time=$EVAL_TIME --gens=$j --num_joints=16
...
python $FILE --run_num="$RUN_NUM" --output_path=$OUT_PATH --validator --eval_time=$EVAL_TIME --gens="$LAST_GEN" --num_joints=16
