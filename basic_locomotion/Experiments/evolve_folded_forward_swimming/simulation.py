...
class Simulation(object):
    ...
    def __init__(self, eval_time, dt, n, man, worm, jointctrlers):
    ...
    def update_callback(self):
        ...
        for jointidx in range(7):
            pos[jointidx] = convert_range(-90, +90, -1, +1, -angle_part)

        pos[7] = convert_range(-90, +90, -1, +1, +45)

        if self.elapsed_time > 3.0:
            assert len(self.jointctrlers) == self.worm._num_joints-8, "number of jointctrlers (%r) must be equal to the number of sinusoidal actuated joints (%r)" % (len(self.jointctrlers),self.worm._num_joints-8)
            for jointidx in range(8,self.worm._num_joints):
                f = self.jointctrlers[jointidx-8].frequency
                ...
                a = self.jointctrlers[jointidx-8].amplitude
                offset = self.jointctrlers[jointidx-8].offset
                phi_val = self.jointctrlers[jointidx-8].phaseshift

                phi = phi_val * (jointidx-8)
