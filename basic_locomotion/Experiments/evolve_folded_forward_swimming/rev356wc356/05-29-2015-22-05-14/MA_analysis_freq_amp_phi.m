...
ylim(limy_freq);
ax = gca;
set(ax,'XTick',xtick,'YTick',ytick_freq,'Fontsize',15);
title('Max fitness frequency progress','Fontsize',22);
...
ylim(limy_ampl);
ax = gca;
set(ax,'XTick',xtick,'YTick',ytick_ampl,'Fontsize',15);
title('Max fitness amplitude progress','Fontsize',22);
xlabel('Generation','Fontsize',20);
...
ylim(limy_ph);
ax = gca;
set(ax,'XTick',xtick,'YTick',ytick_ph,'Fontsize',15);
title('Max fitness phaseshift progress','Fontsize',22);
xlabel('Generation','Fontsize',20);
...
xlim(limy_freq);
for replicatenum = 0:17
    plot(lastgen_freq(replicatenum+1),lastgen_ampl(replicatenum+1),...
        'o','MarkerEdgeColor',colors(replicatenum+1,:),...
        'MarkerFaceColor',colors(replicatenum+1,:),'MarkerSize',5);
end
xlabel('Frequency (Hz)','Fontsize',20);
ylabel('Amplitude','Fontsize',20);
set(gca,'XTick',[0.1:0.3:3,3],'YTick',ytick_ampl,'Fontsize',15);
hold off;
subplot(1,2,2);
hold on;
ylim(limy_ampl);
...
set(gca,'XTick',ytick_ph,'YTick',ytick_ampl,'YTicklabel',[],'Fontsize',15);
legend(legendstr,'location','best','Orientation','horizontal');
hold off;
disp('adjust legend and hit enter');
pause;
fname = exportfname([],'_params.pdf');
export_fig(fname,'-painters','-pdf','-p10');
