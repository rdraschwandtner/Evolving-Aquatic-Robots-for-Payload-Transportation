...
class Simulation(object):
    ...
    def update_callback(self):
        ...
        assert len(self.jointctrlers) == self.worm._num_joints, "number of jointctrlers (%r) must be equal to the number of joints (%r)" % (len(self.jointctrlers),self.worm._num_joints)
        for jointidx in range(self.worm._num_joints):
            f = self.jointctrlers[jointidx].frequency
            ...
            a = self.jointctrlers[jointidx].amplitude
            offset = self.jointctrlers[jointidx].offset
            phi_val = self.jointctrlers[jointidx].phaseshift

            phi = phi_val * (jointidx)
