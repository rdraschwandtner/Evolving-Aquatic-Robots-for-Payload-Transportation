...
class Simulation(object):
    ...
    def update_callback(self):
        ...
        assert self.worm._num_joints % 2 == 0, "this implementation works only with even number of joints: %r" % self.worm._num_joints
        ...
        if self.elapsed_time < 8.0:
            ...
            for jointidx in range(self.worm._num_joints):
                ...
                f = 2
                w = 2*math.pi*f
                #phi = 2*math.pi/self.worm._num_joints * (-jointidx)
                # found the value by trying different phi values and observing the robot
                phi = (2*math.pi/self.worm._num_joints *2.) * (-jointidx)
                jointangle = a * math.sin(w*self.elapsed_time + phi)
                pos.append(jointangle)

        elif self.elapsed_time > 8.0 and self.elapsed_time < 10.0:
            # stop/drifting
            for jointidx in range(self.worm._num_joints):
                a = 0
                phi = 0
            ...
            # inplace turning
            for jointidx in range(int(self.worm._num_joints/2)):
                phi = 2*math.pi/(self.worm._num_joints) * (jointidx-int(self.worm._num_joints/2))

                jointangle = a * math.sin(w*self.elapsed_time+phi) + offset

                pos.append(jointangle)

            for jointidx in range(int(self.worm._num_joints/2),self.worm._num_joints):
                phi = 2*math.pi/(self.worm._num_joints) * (int(self.worm._num_joints/2)-jointidx)
