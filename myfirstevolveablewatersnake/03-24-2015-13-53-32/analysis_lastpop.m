function analysis_lastpop()
    ...
    files = rdir('.\robonode*\**\snake_gen999_indvidx*.json')
    ...
    title('Amplitude distribution per joint (best individual of last generation from 18 replicates)')
    xlabel('Joint number')
    ylabel('Amplitude (deg)')
    
    figure;
    boxplot(lastgen_freq, 'labels', {'1','2','3','4','5','6','7'})
    title('Frequency distribution per joint (best individual of last generation from 18 replicates)')
    xlabel('Joint number')
    ylabel('Frequency (Hz)')
    
    figure;
    boxplot(lastgen_phi .* 180/pi, 'labels', {'1','2','3','4','5','6','7'})
    title('Phi distribution per joint (best individual of last generation from 18 replicates)')
    xlabel('Joint number')
    ylabel('Phi (deg)')
