...
class Simulation(object):
    ...
    def update_callback(self):
        ...
        self.worm.get_sensor_states()

        # state machine
        MOTIONSTATES = {'forward':(0,0.1,self.actuationfreq),'right':(-0.05,1,self.actuationfreq),'left':(0.05,1,self.actuationfreq),'stop':(0.,0.,0.)}
        ...
        pos = []
        for jointidx in range(self.worm._num_joints-1):
            #joint_timeshift = (2.*math.pi/freq) / (self.worm._num_joints) * jointidx
            joint_timeshift = (2.*math.pi/freq) / (self.worm._num_joints) * (jointidx+1)
            phi = joint_timeshift
            assert freq >= 0, "frequency shall not be negative!"
            pos.append(offset + amp * math.sin(2.*math.pi*freq*self.elapsed_time+phi))
            if jointidx == self.worm._num_joints-1: print 'forward'
        
        #for jointidx in range(self.worm._num_joints-1, -1, -1):
        #    #joint_displacement = self.worm.SEG_DIMS[0] * (self.worm._num_joints-1 - jointidx) # reverse joint order for joint displacement

        #    ## one second equals one robotsegment
        #    #joint_timeshift = 1. * joint_displacement

        #    joint_timeshift = (2.*math.pi/freq) / (self.worm._num_joints) * jointidx

        #    # chose times by assuming the wave curve with freq=1Hz
        #    if self.elapsed_time-joint_timeshift >= 4. and self.elapsed_time-joint_timeshift < 12.:
        #        cur_joint_angle = self.worm.get_joint_pos(jointidx) ## todo check function
        #        pos.append(cur_joint_angle)
        #        if jointidx == self.worm._num_joints-1: print 'keep angle'
        #        #bodies = self.worm.get_joint_bodies(jointidx)
        #    else:
        #        phi = joint_timeshift
        #        assert freq >= 0, "frequency shall not be negative!"
        #        pos.append(offset + amp * math.sin(2.*math.pi*freq*self.elapsed_time+phi))
        ...
        #    # map sine phase shift (sec) to distance (meter)
        #    phi = self.worm.SEG_DIMS[0] * jointidx #1 unit := 1 second
        #    assert freq >= 0, "frequency shall not be negative!"
        #    pos.append(offset + amp * math.sin(2.*math.pi*freq*self.elapsed_time+phi))
        #    #self.last_sine_time = self.elapsed_time
        ...
        with open("expected_joint_angles.out", "ab") as csvfile:
            ...
            #spamwriter.writerow([`self.elapsed_time`] + [managerjointactuation_scale(-math.pi, +math.pi, self.expectedpos[angleidx]) for angleidx in range(len(self.expectedpos))])
            spamwriter.writerow([`self.elapsed_time`] + [self.expectedpos])

        self.expectedpos = pos # joint angle next time when update_callback() is called

        #scaled_pos = [managerjointactuation_scale(-math.pi/2, +math.pi/2, pos[angleidx]) for angleidx in range(len(pos))]
        #self.worm.actuate_joints_by_pos(positions=scaled_pos)
