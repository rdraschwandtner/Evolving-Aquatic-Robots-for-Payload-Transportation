function stopping_bodylength()
    ...
    for fidx=1:length(frequencies)
        ...
        massstr = sprintf('mass=%.1f (kg)', totalmass);
        
        currentlegendstr = sprintf('%s (Hz)', frequencies{fidx});
        ...
        title(strcat('distance progress in body lengths', massstr));
        xlabel('time');
        ylabel('body lengths');
        grid on;
        hold off;
        ...
        title(strcat('veloctiy progress, ', massstr));
        xlabel('time');
        ...
        title(strcat('average veloctiy progress, ', massstr));
        xlabel('time');
