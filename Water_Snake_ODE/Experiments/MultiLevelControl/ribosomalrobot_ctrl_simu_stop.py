...
class Simulation(object):
    ...
    def update_callback(self):
        ...
        # state machine
        MOTIONSTATES = {'forward':(0,1,self.actuationfreq),'right':(-0.05,1,self.actuationfreq),'left':(0.05,1,self.actuationfreq),'stop':(0.,0.,0.)}
        ...
        ## swim forward for 5 sec
        ## drift 3 sec
        ## swim reverse in order to stop
        ...
        elif self.elapsed_time >= 5.0 and self.elapsed_time <= 8.0:
            offset, amp, freq = MOTIONSTATES['stop']
            for jointidx in range(self.worm._num_joints-1, -1, -1):
                phi = 2.*math.pi / self.worm._num_joints * jointidx # calc the phase shift for each joint
                #phi += self.elapsed_time - 10.0 # used to hold the same joint position
                assert freq >= 0, "frequency shall not be negative!"
                pos.append(offset + amp * math.sin(2.*math.pi*freq*self.elapsed_time+phi))
            print 'stop'

        elif self.elapsed_time >= 9.0 and self.elapsed_time <= 9.25:
            offset, amp, freq = MOTIONSTATES['forward']
            for jointidx in range(self.worm._num_joints-1 +1):
                phi = 2.*math.pi / self.worm._num_joints * jointidx # calc the phase shift for each joint
                assert freq >= 0, "frequency shall not be negative!"
                pos.append(offset + amp * math.sin(2.*math.pi*freq*self.elapsed_time+phi))
