...
import ribosomalrobot_ctrl_simu
...
if __name__ == '__main__':
    ...
    parser.add_argument("--stop", action="store_true", default=False, help="stop the robot at a certain timestep")
    ...
    if args.stop:
        ...
        GlobalVarWorkaround.worm = ribosomalrobot_ctrl_simu_stop.Worm(man=GlobalVarWorkaround.man, num_joints=7, morphology={'density':density},logging=log_frames, log_path=output_path, logfileprefix='rep' + str(run_num))
        ...
        eval_time = 30.
        GlobalVarWorkaround.man = ODEManager(ribosomalrobot_ctrl_simu.near_callback, stepsize=dt/n, log_data=log_frames, fluid_dynamics=1, gravity=0, run_num=run_num)
        GlobalVarWorkaround.worm = ribosomalrobot_ctrl_simu.Worm(man=GlobalVarWorkaround.man, num_joints=7, logging=log_frames, log_path=output_path, logfileprefix='rep' + str(run_num))
    
        targetposx = args.targetposx
        targetposy = args.targetposy
        print 'target @ x,y (' + `targetposx` + ',' + `targetposy` + ')'
        GlobalVarWorkaround.target = ribosomalrobot_ctrl_simu.Target(man=GlobalVarWorkaround.man, base_pos=[targetposx,1,targetposy])
        ...
        simulation = ribosomalrobot_ctrl_simu.Simulation(eval_time, dt=dt, n=n, man=GlobalVarWorkaround.man, worm=GlobalVarWorkaround.worm, target=GlobalVarWorkaround.target)
