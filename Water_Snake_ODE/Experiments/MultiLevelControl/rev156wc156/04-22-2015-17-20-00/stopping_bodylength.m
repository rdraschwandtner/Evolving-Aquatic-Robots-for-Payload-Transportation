function stopping_bodylength()
    ...
    densities = [50, 500, 5000, 25, 12, 8, 200]
    %densities = [50];
    legendstr = {};
    a = 1;
    for d=densities
    ...
    %% process different fmax differently
        bodyfilename = sprintf('d2000fmax1000body.out', d);
        bodyfile = fopen(bodyfilename);
        bvalues = textscan(bodyfile, 'length:%f;density:%f;dims:[%f, %f, %f];numofsegs:%d');
        fclose(bodyfile);
        blength=bvalues{1};
        ...
        volume = dims(1)*dims(2)*dims(3);
        totalmass = numofsegs*density*volume
        currentlegendstr = sprintf('mass=%.1f (kg) fmax=1000', totalmass);
        legendstr{a} = currentlegendstr;

        positionfilename = sprintf('d2000fmax1000avg_bodypos.out', d);
        inputfile = fopen(positionfilename);
        comvalues = textscan(inputfile, '%f [%f, %f, %f]');
        fclose(inputfile);
        comtimestamp = comvalues{1};
        ...
        grid on;
        hold off;  
    %% stop by no actuation
        bodyfilename = sprintf('d2000fmax1000sbody.out', d);
        bodyfile = fopen(bodyfilename);
        bvalues = textscan(bodyfile, 'length:%f;density:%f;dims:[%f, %f, %f];numofsegs:%d');
        fclose(bodyfile);
        blength=bvalues{1};
        ...
        volume = dims(1)*dims(2)*dims(3);
        totalmass = numofsegs*density*volume
        currentlegendstr = sprintf('mass=%.1f (kg) fmax=1000 s', totalmass);
        legendstr{a} = currentlegendstr;

        positionfilename = sprintf('d2000fmax1000savg_bodypos.out', d);
        inputfile = fopen(positionfilename);
        comvalues = textscan(inputfile, '%f [%f, %f, %f]');
        fclose(inputfile);
        comtimestamp = comvalues{1};
