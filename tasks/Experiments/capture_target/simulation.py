...
def in_circle(worm, bodyidcs, target):
    ...
    return False if (abs(x_cnt) == numbodyseg or abs(z_cnt) == numbodyseg) else True
    ...
    def update_callback(self):
        ...
        if self.grasped == False:
            ...
            if comdist < robotlength:
                ...
                #targetposOpposite = targetpos + [targetpos[0] - com[0], 0, targetpos[2] - com[2]]
                #x = (targetpos[0] - com[0])
                #y = (targetpos[2] - com[2])
                #targetposLeft = [targetpos[0]+x*math.cos(math.pi/4)-y*math.sin(math.pi/4)
                #                 ,targetpos[1]+0
                #                 ,targetpos[2]+y*math.cos(math.pi/4)+x*math.sin(math.pi/4)]
                #normalizedvec_b = (targetposLeft[0] - com[0]
                #                    , targetposLeft[2] - com[2])
                # smoothed_theta = angle2_2d(normalizedvec_a, normalizedvec_b)
                ...
                if comdist < robotlength/math.pi:
                    ...
                    #ampl,freq,phs,gamma_max = MOTIONSTATES['rest']
                    #print 'rest'
                    self.graspcnt = 0
                    self.nextgraspcnt = 0.
                    self.snapdirection[0] = -1 if theta>=0 else +1
                    #self.straightstart = self.elapsed_time if self.straightstart < 0. else self.straightstart
                    #factor = (self.elapsed_time-self.straightstart+1.)*1.

                    #ampl,freq,phs,gamma_max = MOTIONSTATES['right']               
                    #for jointidx in range(self.worm._num_joints):
                    #    #assert freq >= 0, "frequency shall not be negative!"
                    #    #pos[jointidx] = 0.
                    #    pos[jointidx] = (gamma + ampl * math.sin(2.*math.pi*freq*self.elapsed_time-phs*jointidx))#/factor
                    #    if pos[jointidx] < 0.1 and pos[jointidx] > -0.1:
                    #        pos[jointidx] = 0.
                    #print 'rest with ' + str(max(pos))
            ...
            print 'target in circle? ' + str(in_circle(self.worm,range(self.graspcnt,self.worm._num_joints),self.target))
            pos = grasp_algo_wrapper(self.graspcnt,self.worm._num_joints, self.snapdirection[0], self.snapdirection[1])
