...
class ODEManager:
    ...
    def __init__(self, col_callback, stepsize=0.005,log_data=False,output_path="",gravity=-9.81,fluid_dynamics=0,run_num=0,erp=0.5,cfm=1E-4):
        ...
        #self.max_joint_vel = 432000. * self.stepsize*ANG_TO_RAD
        self.max_joint_vel = 80000. * self.stepsize*ANG_TO_RAD # assuming standard servo speed 400 deg/sec
    ...
    def create_sphere(self, key, density, radius, pos, active_surfaces={'x':1,'y':1,'z':1,'-x':1,'-y':1,'-z':1}):
    ...
    def create_capsule(self,key,density,length,radius,pos,base=0,rot=0,R=0., active_surfaces={'x':1,'y':1,'z':1,'-x':1,'-y':1,'-z':1}):
    ...
    def create_box(self,key,dim,pos,terrain=False,density=10.,amp_adj=1.,active_surfaces={'x':1,'y':1,'z':1,'-x':1,'-y':1,'-z':1}):
        """Create a box.
    ...
    def create_surfaces(self,key,amp_adj,active):
        ...
        # Debugging test right now for surfaces.
        # print(key,surfaces)
    ...
    def get_body(self,key):
        """ non-commited function to get the ODE body
        """
        return self.bodies[key]
    
    def create_planarmovingsphere(self, key, density, radius, pos, active_surfaces={'x':1,'y':1,'z':1,'-x':1,'-y':1,'-z':1}):
        """ non-commited function
        """

        key = self.create_sphere(key, density, radius, pos, active_surfaces)
        j = ode.Plane2DJoint(self.world)
        # A body can be attached to the environment by passing None as second body. 
        j.attach(self.bodies[key],None)
